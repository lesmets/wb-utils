...
// The directory page lives in its own root like every tool; it is the one
// root whose page lists all the others.
const DIRECTORY = 'wbest';

// One entry per app directory: the directory page plus every tool.
const roots = fs.readdirSync(ROOT, { withFileTypes: true })
  .filter(d => d.isDirectory() && fs.existsSync(path.join(ROOT, d.name, 'index.html')))
  .map(d => d.name)
  .sort();
if (!roots.includes(DIRECTORY)) fail(`Directory root "${DIRECTORY}/" not found`);
const tools = roots.filter(d => d !== DIRECTORY);
...
const offSite = roots.filter(dir => !sameSite(base.get(dir), new URL(base.get(DIRECTORY)).host));
...
function itemListNames() {
  const graph = jsonld(DIRECTORY)[0]['@graph'];
...
// Locale pages generated by translate.js live in subdirectories of a root
// (wordcounter/de/, wbest/de/, …); each carries its own canonical, which is
// the source of truth here just as the English page's is.
...
function sitemap(dir) {
  // The directory root lists every tool + its own locales; a tool lists
  // itself + locales. Anything off this root's own site is dropped (see
  // sameSite): Search Console refuses the entire sitemap over one foreign URL.
  const siteHost = new URL(base.get(dir)).host;
  const urls = (dir === DIRECTORY
    ? [...roots.map(d => base.get(d)), ...localeUrls(dir)]
...
function llmsTool(dir) {
  ...
  return [
    ...
    `- [WBest](${base.get(DIRECTORY)}): all ${tools.length} free WB tools, private, no sign-up, no uploads.`,
    '',
  ].join('\n');
}
...
function llmsDirectory() {
  const site = jsonld(DIRECTORY)[0]['@graph'].find(n => n['@type'] === 'WebSite');
  const lines = [
    '# WBest',
    '',
    `> ${site.description}`,
  ...
  lines.push('## Frequently asked questions', '', ...faqSection(DIRECTORY));
  return lines.join('\n');
}

function llms(dir) {
  return dir === DIRECTORY ? llmsDirectory() : llmsTool(dir);
}
